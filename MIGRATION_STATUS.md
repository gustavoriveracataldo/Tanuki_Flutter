...
- Detalle de serie con composicion mas cercana a Android: logo o titulo alternativo arriba, caratula grande de la serie a la izquierda, informacion/acciones a la derecha, cast integrado dentro del bloque de informacion, panel glass de episodios a la derecha y feedback TV de foco en filas de episodio.
...
- Panel `Similares` desde el detalle, con titulo/estado y grilla de resultados remotos usando recomendaciones Jikan o busqueda fallback; la ficha de detalle tambien muestra un carrusel horizontal de similares debajo de caratula/informacion.
- Trailers desde detalle, tendencias y busqueda: `trailerUrl` de Jikan/TMDB se conserva en candidatos/series importadas y se abre una cola Flutter multiplataforma con controles anterior/siguiente; en Android/iOS replica el flujo antiguo con WebView embebido para YouTube/Vimeo (`youtube-nocookie`/iframe), mientras escritorio mantiene resolucion a stream directo con `youtube_explode_dart` y `media_kit`; `Abrir externo` queda solo como fallback manual.
...
- Importacion remota conserva el mayor conteo consolidado del buscador/Jikan: si el proveedor scrapeado expone menos episodios que el catalogo, se completan episodios remotos de catalogo para que el detalle no quede truncado, y series ya importadas se expanden al abrirlas desde un resultado con mayor conteo.
...
- Mi espacio con cabecera, resumen, textos de estado y secciones horizontales `Favoritos`, `Quiero ver`, `Viendo`, `Abandonadas` y `Completadas`; deduplica tarjetas por catalogo, titulo limpio, titulo japones y aliases para no repetir la misma serie importada desde varias fuentes.
...
- Enriquecimiento de fichas de catalogo con Jikan para detalle completo, cast y metadata basica por episodio, y enriquecimiento opcional con TMDB/Fanart usando claves por `--dart-define` para poster, fondo, logo, descripcion, rating, trailer y metadata visual. Los candidatos de recomendaciones/random con `mal_id` se completan contra `/v4/anime/{id}/full` antes de importarse para no quedar con un solo episodio cuando la serie tiene mas.
...
- Override TMDB para falsos positivos de remakes confirmados: Saint Seiya clasico/Caballeros del Zodiaco con ano 1985-1990 fuerza `tv/42444` en lugar de `tv/90855`; logos y posters TMDB priorizan idioma japones por grupo antes de votos/tamano. La cache visual paso a prefijo `visual-v4` para no reutilizar arte resuelto antes de este ajuste.
...
- [ ] Auditar y ajustar `Detalle de serie`: poster/fondo/logo, acciones, metadata, lista de episodios, estado/progreso y paneles secundarios. Avance actual: las filas de episodio ya usan la imagen del episodio cuando existe y caen al poster de la serie cuando la metadata remota no trae miniatura; al abrir detalle se refrescan visuales faltantes de series remotas ya importadas. El control `Mi espacio` del detalle replica el patron Android con boton de icono y dialogo de estados, en vez de dropdown visible; ahora cambia icono segun estado y usa radio consistente con los demas botones. La ficha ahora pone caratula grande junto a informacion/acciones, mueve cast al bloque de informacion, muestra carrusel de similares, cambio `Similares` a boton de icono y usa cache visual `visual-v4`. El boton superior de volver conserva la seccion de origen en vez de forzar Inicio. Al seleccionar un episodio se reproduce; al mantenerlo presionado se puede marcar ese capitulo completado o completar capitulos hasta ese punto.
...
- [ ] Revisar escala visual en TV real: se agrego correccion global para Android TV-like por viewport logico y para paneles UHD que reportan viewport bajo, manteniendo monitores 4K sin escala correctiva.
- [ ] Ajustar solo diferencias visuales o de flujo que afecten el 1:1; evitar redisenos.

### P1 - Catalogo, importacion y datos

- [ ] Revisar busqueda/importacion por proveedor activo: aliases, ano, formato, conteo de episodios, duplicados y fallback de catalogo. Avance actual: la importacion y el refresco de series existentes ya no pisan el conteo consolidado con el conteo parcial del proveedor; casos como Samurai X 1996 pueden conservar mas de 90 episodios aunque una fuente liste solo 50. Candidatos abiertos desde `Similares` o `Random` se enriquecen con detalle completo de Jikan antes de importarse para evitar series truncadas a un capitulo.
...
- [ ] Revisar scripts tras mover el proyecto de disco/ruta. Avance actual: `scripts/build_linux.sh` detecta `CMakeCache.txt` con ruta absoluta antigua y cache `MEDIA_KIT_LIBS_AVAILABLE=OFF`, y borra la cache Linux release antes de compilar.
...
Ultima actualizacion: trailers prueban `video_player` con backend `fvp` antes de `media_kit`/WebView, fila `Ultimas peliculas` con carga dedicada de peliculas y correccion de escala mas agresiva para Android TV-like.
...
- Revision estatica de `playlist_engine.dart`, `models.dart`, `app_controller.dart`, `player_screen.dart`, `trailer_queue_screen.dart` y `remote_catalog_service.dart`. No se ejecuto `dart format`, `flutter analyze` ni builds porque `dart`/`flutter` no estan disponibles en el PATH de esta shell.
...
- Revision estatica de `app_controller.dart` para prefijo `visual-v4` en cache de candidatos.
- `dart format lib/src/services/remote_catalog_service.dart lib/src/app_controller.dart`
- `dart analyze lib/src/services/remote_catalog_service.dart lib/src/app_controller.dart` (sin issues).
...
- Revision estatica de `app_controller.dart` para reemplazo de poster base por cache visual `visual-v4`.
- Revision estatica de `remote_catalog_service.dart` para completar episodios faltantes con placeholders de catalogo cuando Jikan/buscador trae mayor conteo que la fuente scrapeada.
- Revision estatica de `app_controller.dart` para expandir series remotas ya importadas cuando un resultado consolidado trae mayor conteo.
- `dart format lib/src/services/remote_catalog_service.dart lib/src/app_controller.dart`
- `dart analyze lib/src/services/remote_catalog_service.dart lib/src/app_controller.dart` (sin issues).
