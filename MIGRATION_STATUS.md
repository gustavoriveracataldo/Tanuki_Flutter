...
Ultima actualizacion: version 0.9.0, playlist con modo `TV`/`Por serie`, dedupe de series equivalentes en playlist, salida automatica de playlist al completar serie, acciones de completado desde episodios del detalle, fila `Ultimas peliculas`, autor/version movidos a Ajustes, tarjetas `A continuacion`/`Mas tarde` secuenciales y trailers YouTube con intento directo `media_kit` antes del WebView Android/iOS.
