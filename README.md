...
### Opcional: TMDB/Fanart/MAL/SIMKL

Si quieres que las series importadas completen poster, fondo, logo, descripcion, rating y trailer usando TMDB/Fanart, define estas variables antes de generar dist:

```sh
export TMDB_API_KEY="tu_api_key"
# o, alternativamente:
export TMDB_BEARER_TOKEN="tu_bearer_token"
export FANART_API_KEY="tu_fanart_key"
export MYANIMELIST_CLIENT_ID="tu_mal_client_id"
export MYANIMELIST_CLIENT_SECRET="tu_mal_client_secret"
export SIMKL_CLIENT_ID="tu_simkl_client_id"
```

Los scripts `scripts/build_android.sh`, `scripts/build_linux.sh` y `scripts/build_windows.ps1` pasan esas variables automaticamente como `--dart-define` cuando existen. MAL y SIMKL tambien pueden configurarse desde Ajustes sin recompilar. Si usas `flutter build ...` directo, agrega manualmente los `--dart-define`.
...
- Android: `tanuki-android-v0.8.7.apk`
- Linux x64: `tanuki-linux-x64-v0.8.7.tar.gz`
- Windows x64: `tanuki-windows-x64-v0.8.7.zip`
...
### Secrets opcionales para Actions

Si quieres que los builds de release incluyan integraciones de metadata/sync, agrega estos secrets en GitHub:
...
Si no estan configurados, la app compila igual y esas integraciones quedan disponibles para configuracion manual desde Ajustes o para builds locales con variables de entorno.
